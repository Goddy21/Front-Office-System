...
router.post('/agreement/complete-step', upload.fields([
    { name: 'songs', maxCount: 5 },
    { name: 'artwork', maxCount: 1 }
  ]), async (req, res) => {
    const { visitorId, final_state } = req.body;
    const completedSteps = req.body.steps || [];
  
    try {
      // Handle completion of each step
      if (Array.isArray(completedSteps)) {
        for (const step of completedSteps) {
          await pool.query(`UPDATE agreement_steps SET completed = true WHERE visitor_id = $1 AND step_name = $2`, [visitorId, step]);
        }
      } else if (typeof completedSteps === 'string') {
        await pool.query(`UPDATE agreement_steps SET completed = true WHERE visitor_id = $1 AND step_name = $2`, [visitorId, completedSteps]);
      }
  
      // Save final state
      await pool.query(`UPDATE visitors SET status = $1 WHERE id = $2`, [final_state, visitorId]);
  
      res.send('Agreement steps updated successfully.');
    } catch (err) {
      console.error('Error updating contract agreement steps:', err);
  ...
  router.get('/start-contract/:id', async (req, res) => {
    ...
    try {
        ...
        html += `
                    ...
                    <form method="POST" action="/agreement/complete-step" enctype="multipart/form-data">
